...
lower_frame.place(relx=0.5, rely=0.25, relwidth=0.59, relheight=0.6, anchor='n')
...
lower_frame1.place(relx=0.5, rely=0.25, relwidth=0.59, relheight=0.6, anchor='n')
...
labeltext2 = Label(lower_frame1, text=u"Привет, это второй раздел учебника.\n В tkinter от класса Canvas создаются объекты-холсты, на которых можно рисовать,\n"
                              ...
                              "холсте. Точкой отсчета является верхний левый угол.", font=("Times New Roman", 14), bg="white")
...
labelimage1.place(relx=0.5, rely=0.30, relwidth=1, relheight=0.4, anchor='n')
labeltext2 = Label(lower_frame1, text="В программе ниже создается холст.\n"
                                      ...
                                      "c.pack()\n"
                                      "window.mainloop()\n"
                                      "в следующей главе мы разберем как рисовать на этом холсте", font=("Times New Roman", 14), bg="white")
labeltext2.place(relx=0.5, rely=0.65, relwidth=1, relheight=0.35, anchor='n')
...
lower_frame2.place(relx=0.5, rely=0.1, relwidth=0.59, relheight=0.8, anchor='n')
labeltext3 = Label(lower_frame2, text="В tkinter уже есть графические примитивы, для рисования, их нужно всего лишь правильно "
                              ...
                              "y2) В программе ниже создаётся и рисуется линия на холсте.", font=("Times New Roman", 14), bg="white")
labeltext3.place(relwidth=1, relheight=0.3)
codeimg = PhotoImage(file='code.pgm')
labelimg = Label(lower_frame2, bg='white', image=codeimg)
labelimg.place(relx=0.5, rely=0.20, relwidth=1, relheight=0.4, anchor='n')
...
rectanglelabel.place(relx=0.5, rely=0.60, relwidth=1, relheight=0.3, anchor='n')
labelgotext2 = Label(lower_frame2, text="Далее о других примитивах в следующей вкладке", font=("Times New "
                                                                                                    "Roman", 14),
                     bg='white')
labelgotext2.place(relx=0.5, rely=0.9, relwidth=1, relheight=0.1, anchor='n')
...
lower_frame3.place(relx=0.5, rely=0.01, relwidth=0.59, relheight=1, anchor='n')

oval = PhotoImage(file='oval_1.ppm')
ovallabel = Label(lower_frame3,bg='white', image=oval)
ovallabel.place(relx=0.5, rely=0, relwidth=1, relheight=0.45, anchor='n')

elipsoid = PhotoImage(file='ellips.ppm')
elabel = Label(lower_frame3,bg='white', image=elipsoid)
elabel.place(relx=0.5, rely=0.4, relwidth=1, relheight=0.25, anchor='n')

labeltext4 = Label(lower_frame3, text="Метод create_oval(x1, y1, x2, y2)\n создает эллипсы. При этом задаются\n координаты гипотетического "
                              "прямоугольника, описывающего эллипс. Если нужно\n получить круг, то соответственно "
                              "описываемый прямоугольник должен быть квадратом.\n"
                                      "Методом create_polygon(x1, x2...xn, yn)\n рисуется произвольный многоугольник\n путем задания координат каждой его точки\n"
                              ...
                              "activeoutline определяет цвет обводки при наведении курсор", font=("Times New Roman", 14),
                   bg="white")
labeltext4.place(relx=0.5, rely=0.6, relwidth=1, relheight=0.4, anchor='n')
...
lower_frame4.place(relx=0.5, rely=0.25, relwidth=0.59, relheight=0.6, anchor='n')
...
labeltext5 = Label(lower_frame4, text="Более сложные для понимания фигуры получаются при использовании метода create_arc(). В \n"
                              ...
                              "угол поворота.",
                   font=("Times New Roman", 14), bg="white")
labeltext5.place(relwidth=1, relheight=0.3)

arc = PhotoImage(file='arc.ppm')
arclabel = Label(lower_frame4,bg='white', image=arc)
arclabel.place(relx=0.5, rely=0.25, relwidth=1, relheight=0.4, anchor='n')

arc2 = PhotoImage(file='arc2.ppm')
arclabel2 = Label(lower_frame4,bg='white', image=arc2)
arclabel2.place(relx=0.5, rely=0.65, relwidth=1, relheight=0.4, anchor='n')
...
tablelabel.place(relx=0.5, rely=0.1, relwidth=0.55, relheight=0.7, anchor='n')

tab_control.add(tab7, text='Практикум')

background_image7 = PhotoImage(file='background.ppm')
background_label7 = Label(tab7, image=background_image7)
background_label7.place(relwidth=1, relheight=1)

lower_frame7 = Frame(tab7, bg="lightblue", bd=10)
lower_frame7.place(relx=0.5, rely=0.05, relwidth=0.59, relheight=0.9, anchor='n')

labelTASK1 = Label(lower_frame7, text="1) Пропеллер\n"
                              "Нарисуйте пропеллер, как это показано ниже\n"
                                      "'Кто мечтает быть пилотом, очень смелый видно тот. От-от-от вин-та!'", font=("Georgia", 18, 'bold'), bg='white')

labelTASK1.place(relwidth=1, relheight=0.2)

propeller = PhotoImage(file='propeller.ppm')
propelabel = Label(lower_frame7, bg='white', image=propeller)
propelabel.place(relx=0.5, rely=0.16, relwidth=1, relheight=0.45, anchor='n')
...
labelTASK2 = Label(lower_frame7, text="2) Торт\n"
                              ...
                              "'Кто сьел мой двумерный массив?!'", font=("Georgia", 18, 'bold'), bg='white')
labelTASK2.place(relx=0.5, rely=0.6, relwidth=1, relheight=0.2, anchor='n')

tort = PhotoImage(file='tort.ppm')
tortlabel = Label(lower_frame7, bg='white', image=tort)
tortlabel.place(relx=0.5, rely=0.78, relwidth=1, relheight=0.25, anchor='n')
...
labelanimation = Label(lower_frame8, text='Методы, создающие фигуры на холсте, возвращают численные идентификаторы \n'
                                          ...
                                          "миллисекунд.\n В следующей вкладке разберём это подробно",  font=("Times New Roman", 13),
                   bg="white")
...
labelanimation3 = Label(lower_frame9, text='В данном коде создаётся шар, который двигается. Вначале происходит '
                                           ...
                                           'После чего с помощью motion(), запускаем нашу функцию и само окно tkinter.', font=("Times New Roman", 14),
                   bg="white")
